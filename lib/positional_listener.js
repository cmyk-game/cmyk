...
module.exports = function(game,remoteEmitter) {
    ...
    forwardEvents('switchOn')
    forwardEvents('enter')
    forwardEvents('exit')
    function forwardEvents(eventName){
        positional.on(eventName,function(pos){
            remoteEmitter.emit('positional',{
                eventName:eventName,
                pos:pos,
            })
        })
    }
    remoteEmitter.on('positional',function(data){
        positional.emit(data.eventName,data.pos)
    })
